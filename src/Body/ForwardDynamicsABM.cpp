...
void ForwardDynamicsABM::updateForceSensor(ForceSensor* sensor)
{
    ...
    const Matrix3 sensorR = link->R * sensor->localR;
    const Vector3 fs = sensorR.transpose() * f;
    const Vector3 sensorPos = link->p + link->R * sensor->localPos;
    const Vector3 ts = sensorR.transpose() * (tau - sensorPos.cross(f));
