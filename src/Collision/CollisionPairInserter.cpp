...
int CollisionPairInserter::is_convex_neighbor(col_tri* t1, col_tri* t2)
{
    ...
    if(t1->n.dot(vec1) < EPS && t1->n.dot(vec2) < EPS && t1->n.dot(vec3) < EPS){
        return 1;
    } else {
        return 0;
    }
...
(col_tri* tri_convex_neighbor, col_tri* tri_neighbor, int* start_tri, int* end_tri, int num_tri)
{
...
(tri* root, col_tri* tri_convex_neighbor, col_tri* tri_neighbor, int num_tri, int max_num)
{
    ...
    Opcode::MeshInterface* mesh2)
{
    ...
    Vector3 signed_distance1 = Vector3::Constant(99999999.0);
    Vector3 signed_distance2 = Vector3::Constant(-99999999.0);
...
int CollisionPairInserter::new_point_test(int k)
{
    ...
    for(int i=0; i < last; ++i){
        for(int j=0; j < cdContact[i].num_of_i_points; ++j){
            const Vector3 dv = cdContact[i].i_points[j] - cdContact[last].i_points[k];
            double d = cdContact[i].depth - cdContact[last].depth;
            if(dv.squaredNorm() < eps && d*d < eps) return 0;
        }
    }
    return 1;
}
