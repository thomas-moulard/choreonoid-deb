...
(BVTCache& cache, float& minD, Point &point0, Point&point1,
...
void SSVTreeCollider::Distance
(const AABBCollisionTree* tree0, const AABBCollisionTree* tree1, 
 const Matrix4x4* world0, const Matrix4x4* world1, 
...
(BVTCache& cache, double tolerance, const Matrix4x4* world0, const Matrix4x4* world1)
{
    // Checkings
    if(!cache.Model0 || !cache.Model1)								return false;
    if(cache.Model0->HasLeafNodes()!=cache.Model1->HasLeafNodes())	return false;
...
bool SSVTreeCollider::Collide
(const AABBCollisionTree* tree0, const AABBCollisionTree* tree1, 
 const Matrix4x4* world0, const Matrix4x4* world1, Pair* cache, double tolerance)
{
    // Init collision query
    InitQuery(world0, world1);
...
float SSVTreeCollider::SsvSsvDist
(const AABBCollisionNode *b0, const AABBCollisionNode *b1)
{
	float dist = 0.0;
    ...
    if(t1 == CollisionAABB::SSV_PSS && t2 == CollisionAABB::SSV_PSS){
        dist = PssPssDist(b0->mAABB.mRadius, b0->mAABB.mCenter, 
                          b1->mAABB.mRadius, b1->mAABB.mCenter);
    } else if(t1 == CollisionAABB::SSV_PSS && t2 == CollisionAABB::SSV_LSS){
        dist = PssLssDist(b0->mAABB.mRadius, b0->mAABB.mCenter,
                          b1->mAABB.mRadius, b1->mAABB.mPoint0, b1->mAABB.mPoint1);
    } else if(t1 == CollisionAABB::SSV_LSS && t2 == CollisionAABB::SSV_PSS){
        dist = LssPssDist(b0->mAABB.mRadius, b0->mAABB.mPoint0, b0->mAABB.mPoint1,
                          b1->mAABB.mRadius, b1->mAABB.mCenter);
    } else if(t1 == CollisionAABB::SSV_LSS && t2 == CollisionAABB::SSV_LSS){
        dist = PssPssDist(sqrtf(b0->GetSize()), b0->mAABB.mCenter, 
                          sqrtf(b1->GetSize()), b1->mAABB.mCenter);
    } else {
        throw "this ssv combination is not supported";
    }
	return dist;
}

float SSVTreeCollider::PssPssDist
(float r0, const Point& center0, float r1, const Point& center1)
{
    Point c0;
...
float SSVTreeCollider::PssLssDist
(float r0, const Point& center0, float r1, const Point& point0, const Point& point1)
{
    Point c0;
    TransformPoint(c0, center0, mR0to1, mT0to1);
...
float SSVTreeCollider::LssPssDist
(float r0, const Point& point0, const Point& point1, float r1, const Point& center0)
{
    Point p0, p1;
...
float SSVTreeCollider::LssLssDist
(float r0, const Point& point0, const Point& point1, float r1, const Point& point2, const Point& point3)
{
    Point p0, p1;
    TransformPoint(p0, point0, mR0to1, mT0to1);
    TransformPoint(p1, point1, mR0to1, mT0to1);
...
void SSVTreeCollider::_Distance
(const AABBCollisionNode* b0, const AABBCollisionNode* b1, float& minD, Point& point0, Point& point1)
{
...
bool SSVTreeCollider::_Collide
(const AABBCollisionNode* b0, const AABBCollisionNode* b1, double tolerance)
{
    mNowNode0 = b0;
    mNowNode1 = b1;
    float d;
